...
const snippetCode = `import figlet from 'figlet';
...
const snippetVars = computed(() => ({
  input: input.value,
  font: font.value,
  output: output.value ? '(ascii art rendered above)' : '(loading...)',
}));
...
<template>
  <c-card style="max-width: 600px;">
    <c-input-text
      v-model:value="input" label="Your text:" placeholder="Your text to draw" raw-text multiline
      rows="4"
    />
    ...
    <n-grid cols="4" x-gap="12" w-full>
      <n-gi span="2">
        <c-select
          v-model:value="font" label-position="top" label="Font:" :options="fonts" searchable="true"
    ...
    <n-form-item v-if="!processing && !errored" label="Ascii Art text:">
      <CodeSnippet :value="output" mb-1 mt-1 copy-placement="outside" />
    </n-form-item>
  </c-card>
